...
## 実機目視レビューで発見・修正した事項（5/17・ローカルでないと出ない実バグ）

全20P＋FVを desktop/mobile スクショ実機レビュー。確定デザインは全実コンテンツで
提示品質で再現を確認。その過程で**プロト単体では出ない実SWELL固有の不具合**を発見・修正:

1. **FV配下の詳細度負け（重要・本番にも効く修正）**:
   FVは固定ページ本文として置くため SWELL の `.post_content` 配下に入る。
   `.ots-fv__call` 等(0,0,1,0)が SWELL の `.post_content p`(0,0,1,1)に詳細度で負け、
   縦書きが**淡色＝低コントラスト**になっていた。
   → 全 `.ots-fv__*` を `.post_content .ots-fv__*` スコープ化＋FV内 `p` の余白/行間を
     リセット（色は各ブロック別ルールで指定）。lead=#972C00 / call=#1A1A1A /
     close=#3A3632 / sign=#6B6058 を計算値で確認。子テーマに恒久反映済
2. **SWELL既定フロントUIの抑止**:
   フロントに SWELL 既定の メインビジュアル(`.p-mainVisual` 自転車/砂漠の仮画像
   "さぁ、始めよう。")／ピックアップ／投稿リスト("Hello world!")／サイドバーが出る。
   → 子テーマ §7 で `.home` 配下を display:none ＋ `.ots-fv` を 100vw フルブリード化。
   サンプル投稿/ページも削除。
   **本番さくらでは同等を SWELL カスタマイザーで設定**（メインビジュアル非表示／
   フロント1カラム）するのが正道。CSS抑止はローカル確定再現用。
3. **インナーページのサイドバー**: サービス/会社ページに SWELL 既定サイドバー。
   1カラム化は本番ビルド時の SWELL 設定判断（CSS強制しない＝先方が連絡導線等を
   入れる可能性もあるため）。要・先方/ビルド時決定。
4. **旧コンテンツ起因（CSS対象外・書き換え禁止）**: `strong` 多用ページ(代表挨拶等)で
   深オレンジ強調が多い／`securitycamera` の旧inline画像が不揃い。これらは2014年
   コンテンツのHTML構造由来。整列は**コンテンツ再構築＝テーマ移管スコープ外**。
   CLAUDE.md「提供テキストを勝手に書き換えない」に従い原則保持。

## 本番（さくら）への移し方（このローカル成果の使い道）

このローカル環境＝**安全な実装・検証場**。確定後の本番移行は:
1. さくらでバックアップ多重取得（CLAUDE.md既定）→ サブディレクトリ/ステージング複製
2. 本ローカルで確定した子テーマ `swell_child/` をそのまま転送（FV修正＝詳細度対応込み）
3. WXR取込 → `http://→https://www.trust-supply.com` を WP-CLI `search-replace`（シリアライズ安全）
4. **SWELLカスタマイザー設定**: メインビジュアル「表示しない」／フロントページ1カラム
   （子テーマ §7 の `.home` CSS抑止と等価。本番は設定で正道に）。FVは固定ページ
   「ホーム」本文に `.ots-fv` マークアップ（雛形＝`site/proto/plan-a/index.html` /
   作成済HTMLは local setup の `/tmp/ots_fv.html` 相当）を入れ front page に指定
5. pretty permalink（本番Apacheは mod_rewrite 有効）→ 全20P＋親 curl で 200/301 全数確認
6. CF7・GA4再設置・本番切替（パーマリンク不変）

## 検証スクショ

`bash site/local-staging/setup.sh` 後、`wp-inventory/shot-local.mjs` で全20P＋FVを
