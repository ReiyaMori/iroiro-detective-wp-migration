...
| 🟠 先方待ち(非ブロッカー) | 現行料金表の確定値 | ⏳ **5/19「確認します」**＝コア4点で唯一の実質未回答。トップ料金はプレースホルダ継続・受領後差替 |
| ✅ 本番切替の作業時間帯 | 早朝 or 夜間（先方こだわりなし） | ✅ **5/19先方回答**＝Jの唯一のハード前提クリア。具体日時は料金が揃い次第すり合わせ（当初目安日から数日前後の余地は通知済） |
| ✅ 法務(必須) | 探偵業届出証明書番号（探偵業法§10） | ✅ **5/19受領＝第93090018号（熊本県公安委員会）**。footer-content.html / components.html 反映済 |
| ✅ /flow(相談の流れ)公開要否 | 現状どおり非公開で確定 | ✅ **5/19先方回答＝新サイトでも非公開**。pages WXR未収録のままで整合＝移管対象外（追加作業なし） |

- **A〜I（事前アップロード含む）は先方待ち・SSHと無関係に着手可**（非破壊）。
- **J（本番反映）だけが不可逆ポイント**。前提＝**先方合意の作業時間帯**は
  **5/19クリア済**（早朝 or 夜間／先方こだわりなし）。残るは具体日時のすり合わせのみ＝
  「料金確定値の受領」→「実施日時を先方とすり合わせ」→Jの順。料金は非ブロッカー
  （プレースホルダで先行公開しトップ料金のみ後日差替も可＝先方判断）。
...
- ~~本番切替の作業時間帯~~ → ✅ **5/19クリア＝早朝/夜間**（具体日時は料金受領後すり合わせ）
- **現行料金表の確定値**（トップ料金プレースホルダ差替）＝**5/19「確認します」＝唯一の実質先方待ち**
